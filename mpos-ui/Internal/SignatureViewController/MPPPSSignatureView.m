...
struct MPPPSSignaturePoint
{
    GLKVector3		vertex;
    GLKVector3		color;
};
typedef struct MPPPSSignaturePoint MPPPSSignaturePoint;


// Maximum verteces in signature
static const int mp_maxLength = MAXIMUM_VERTECES;


// Append vertex to array buffer
static inline void mp_addVertex(uint *length, MPPPSSignaturePoint v) {
    if ((*length) >= mp_maxLength) {
        return;
    }
    
    GLvoid *data = glMapBufferOES(GL_ARRAY_BUFFER, GL_WRITE_ONLY_OES);
    memcpy(data + sizeof(MPPPSSignaturePoint) * (*length), &v, sizeof(MPPPSSignaturePoint));
    glUnmapBufferOES(GL_ARRAY_BUFFER);
    
    (*length)++;
}

static inline CGPoint mp_QuadraticPointInCurve(CGPoint start, CGPoint end, CGPoint controlPoint, float percent) {
...
static float mp_generateRandom(float from, float to) { return random() % 10000 / 10000.0 * (to - from) + from; }
static float mp_clamp(float min, float max, float value) { return fmaxf(min, fminf(max, value)); }


// Find perpendicular vector from two other vectors to compute triangle strip around line
static GLKVector3 mp_perpendicular(MPPPSSignaturePoint p1, MPPPSSignaturePoint p2) {
    GLKVector3 ret;
...
static MPPPSSignaturePoint mp_ViewPointToGL(CGPoint viewPoint, CGRect bounds, GLKVector3 color) {
    
    return (MPPPSSignaturePoint) {
        {
            {
...
@interface MPPPSSignatureView () {
    ...
    MPPPSSignaturePoint SignatureVertexData[mp_maxLength];
    uint length;
    
    MPPPSSignaturePoint SignatureDotsData[mp_maxLength];
    uint dotsLength;
    ...
    MPPPSSignaturePoint previousVertex;
    MPPPSSignaturePoint currentVelocity;
}

@end


@implementation MPPPSSignatureView
...
- (void)tap:(UITapGestureRecognizer *)t {
    ...
    if (t.state == UIGestureRecognizerStateRecognized) {
        ...
        MPPPSSignaturePoint touchPoint = mp_ViewPointToGL(l, self.bounds, (GLKVector3){{1, 1, 1}});
        mp_addVertex(&dotsLength, touchPoint);
        
        MPPPSSignaturePoint centerPoint = touchPoint;
        centerPoint.color = StrokeColor;
        mp_addVertex(&dotsLength, centerPoint);
        
        static int segments = 20;
        GLKVector2 radius = (GLKVector2){
            {
                mp_clamp(0.00001, 0.02, penThickness * mp_generateRandom(0.5, 1.5)),
                mp_clamp(0.00001, 0.02, penThickness * mp_generateRandom(0.5, 1.5))
            }
        };
...
- (void)pan:(UIPanGestureRecognizer *)p {
    ...
    currentVelocity = mp_ViewPointToGL(v, self.bounds, (GLKVector3){{0,0,0}});
    float distance = 0.;
    ...
    float clampedVelocityMagnitude = mp_clamp(VELOCITY_CLAMP_MIN, VELOCITY_CLAMP_MAX, velocityMagnitude);
    ...
    if ([p state] == UIGestureRecognizerStateBegan) {
        ...
        MPPPSSignaturePoint startPoint = mp_ViewPointToGL(l, self.bounds, (GLKVector3){{1, 1, 1}});
        previousVertex = startPoint;
        previousThickness = penThickness;
        
        mp_addVertex(&length, startPoint);
        mp_addVertex(&length, previousVertex);
        ...
        if (distance > QUADRATIC_DISTANCE_TOLERANCE) {
            ...
            for (i = 0; i < segments; i++)
            {
                ...
                CGPoint quadPoint = mp_QuadraticPointInCurve(previousMidPoint, mid, previousPoint, (float)i / (float)(segments));
                
                MPPPSSignaturePoint v = mp_ViewPointToGL(quadPoint, self.bounds, StrokeColor);
            ...
            MPPPSSignaturePoint v = mp_ViewPointToGL(l, self.bounds, StrokeColor);
            [self addTriangleStripPointsForPrevious:previousVertex next:v];
        ...
        MPPPSSignaturePoint v = mp_ViewPointToGL(l, self.bounds, (GLKVector3){{1, 1, 1}});
        mp_addVertex(&length, v);
...
- (void)bindShaderAttributes {
    ...
    glVertexAttribPointer(GLKVertexAttribPosition, 3, GL_FLOAT, GL_FALSE, sizeof(MPPPSSignaturePoint), 0);
...
- (void)addTriangleStripPointsForPrevious:(MPPPSSignaturePoint)previous next:(MPPPSSignaturePoint)next {
    float toTravel = penThickness / 2.0;
    
    for (int i = 0; i < 2; i++) {
        GLKVector3 p = mp_perpendicular(previous, next);
        ...
        MPPPSSignaturePoint stripPoint = {
            { {p1.x + difX, p1.y + difY, 0.0} },
            StrokeColor
        };
        mp_addVertex(&length, stripPoint);
